...
route.get('/specReport', async (req, res) => 
{
    try 
    {
        const academicYear = await academic.findOne({
            where: { active_sem: 1 },
        ...
        const deptList = await markentry.findAll({
            where: { academic_year: academicYear.academic_year },
            attributes: ['dept_id'],
            group: ['dept_id'],
            raw: true
        });
        ...
        for (const { dept_id } of deptList) {
            ...
            const uniqueCourseCode = [...new Set(courseEntries.map(code => code.course_code))];
            ...
            for (const course_code of uniqueCourseCode) {
                ...
                const cop = await rsmatrix.findAll({ where: { course_code } });
                ...
                for (const entry of cop) {
                    const denominator1 = entry.co1_pso1 + entry.co2_pso1 + entry.co3_pso1 + entry.co4_pso1 + entry.co5_pso1 || 1;
                    const denominator2 = entry.co1_pso2 + entry.co2_pso2 + entry.co3_pso2 + entry.co4_pso2 + entry.co5_pso2 || 1;
                    const denominator3 = entry.co1_pso3 + entry.co2_pso3 + entry.co3_pso3 + entry.co4_pso3 + entry.co5_pso3 || 1;
                    const denominator4 = entry.co1_pso4 + entry.co2_pso4 + entry.co3_pso4 + entry.co4_pso4 + entry.co5_pso4 || 1;
                    const denominator5 = entry.co1_pso5 + entry.co2_pso5 + entry.co3_pso5 + entry.co4_pso5 + entry.co5_pso5 || 1;

                    const capso1 = ((lot * entry.co1_pso1) + (lot * entry.co2_pso1) + (mot * entry.co3_pso1) + (mot * entry.co4_pso1) + (hot * entry.co5_pso1)) / denominator1;
                    const capso2 = ((lot * entry.co1_pso2) + (lot * entry.co2_pso2) + (mot * entry.co3_pso2) + (mot * entry.co4_pso2) + (hot * entry.co5_pso2)) / denominator2;
                    const capso3 = ((lot * entry.co1_pso3) + (lot * entry.co2_pso3) + (mot * entry.co3_pso3) + (mot * entry.co4_pso3) + (hot * entry.co5_pso3)) / denominator3;
                    const capso4 = ((lot * entry.co1_pso4) + (lot * entry.co2_pso4) + (mot * entry.co3_pso4) + (mot * entry.co4_pso4) + (hot * entry.co5_pso4)) / denominator4;
                    const capso5 = ((lot * entry.co1_pso5) + (lot * entry.co2_pso5) + (mot * entry.co3_pso5) + (mot * entry.co4_pso5) + (hot * entry.co5_pso5)) / denominator5;

                    totalCapso1 += capso1; totalCapso2 += capso2; totalCapso3 += capso3;
                    totalCapso4 += capso4; totalCapso5 += capso5;
...
async function calculateCategory(percentage) 
{
    ...
    catch (error) { console.error('Error fetching academic or calculation data:', error) }
}
